...
public class ItemControllerController {
    ...
    @GetMapping(value = "/")
    public ResponseEntity<Page<Item>> list(@RequestParam(required = false) Integer current, @RequestParam(required = false) Integer pageSize) {
        if (current == null) {
            current = 1;
        }
        if (pageSize == null) {
        ...
        Page<Item> aPage = itemService.page(new Page<>(current, pageSize));
    ...
    public ResponseEntity<Object> update(@RequestBody Item params) {
